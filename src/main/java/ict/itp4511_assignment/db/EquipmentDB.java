...
public class EquipmentDB {
    ...
    public void insertEquipmentRecord() {
        ...
        try {
            ...
            String sql = "INSERT INTO equipment (equipmentName, equipmentType ,serialNumber, purchaseDate, warrantyPeriod, status,isPrivate, campusID) " + "VALUES ('MacBook Pro 16-inch', 'Laptop',  'SN001', '2024-01-10', 12, 'Available',0, 'TY')," + "('MacBook Air 13-inch', 'Laptop', 'SN002', '2024-01-15', 12, 'CheckedOut',0, 'TY')," + "('iPad Pro 12.9-inch', 'Tablet','SN003', '2024-02-01', 12, 'Available',0, 'TY')," + "('iPad mini 8.3-inch', 'Tablet',  'SN004', '2024-02-05', 12, 'CheckedOut',0, 'TY')," + "('Canon PIXMA MG3620', 'Printer and Scanner',  'SN005', '2024-03-01', 24, 'Available',0, 'TY')," + "('Canon imageCLASS MF743Cdw', 'Printer and Scanner',  'SN006', '2024-03-05', 24, 'Available', 0,'CW')," + "('iPhone 13 Pro Max', 'Smartphone',  'SN007', '2024-04-01', 12, 'Available', 0,'TY')," + " ('iPhone SE', 'Smartphone',  'SN008', '2024-04-05', 12, 'Available', 0,'TY')," + "('Dell UltraSharp 27', 'Monitor','SN011', '2024-06-01', 36, 'Available',1, 'CW')," + " ('Dell P Series 24', 'Monitor',  'SN012', '2024-06-05', 36, 'Available', 0,'TY')," + "('LEGO Mindstorms EV3', 'Robot',  'SN013', '2024-07-01', 24, 'Available', 1,'TY')," + "('LEGO Boost Creative Toolbox', 'Robot', 'SN014', '2024-07-05', 24, 'Available',0, 'TM')," + "('HP ProLiant DL380 Gen10', 'Server',  'SN015', '2024-08-01', 36, 'Available',1, 'TY')," + "('HP ProLiant ML350 Gen10', 'Server',  'SN016', '2024-08-05', 36, 'Available', 1,'CW')," + "('Cisco 4000 Series Integrated Services Router', 'Networking', 'SN017', '2024-09-01', 48, 'Available', 1,'TY')," + "('Cisco Catalyst 2960-X Series Switches', 'Networking', 'SN018', '2024-09-05', 48, 'Available', 0,'TY')," + "('Bose L1 Compact Portable Line Array System', 'Audio-Visual', 'SN019', '2024-10-01', 24, 'Available',0, 'TY')," + " ('Bose S1 Pro Multi-Position PA System', 'Audio-Visual',   'SN020','2024-10-05', 24, 'Available', 0,'TY')," + "('Wacom Intuos Pro', 'Others','SN021', '2024-11-01', 24, 'Available', 0,'TY')," + "('Wacom Cintiq 22', 'Others','SN022', '2024-11-05', 24, 'Available', 0,'LWL');";
    ...
    public ArrayList<WishCartEquipmentBean> getEquipmentList(int userId, String userType, String campus) {
        ...
        try {
            ...
            if (Objects.equals(userType, "User")) {
                sql = "SELECT * " + "FROM wishlist\n" + "         RIGHT JOIN equipment\n" + "                    ON wishlist.equipmentID = equipment.equipmentID " + "         LEFT JOIN reserveCart" + "                   ON equipment.equipmentID = reserveCart.equipmentID " + "WHERE (equipment.status = 'Available' OR equipment.status = 'CheckedOut') " + "and isPrivate = 0 ";
...
//                sql = "SELECT   * FROM wishlist" + " RIGHT JOIN " + "equipment" + " ON" + " equipment.equipmentID = wishlist.equipmentID" + " WHERE (status = 'Available' OR status = 'CheckedOut')" + " AND (userID = ? OR userID IS NULL) AND campusID=?";
                sql = "SELECT * " + "FROM wishlist\n" + "         RIGHT JOIN equipment\n" + "                    ON wishlist.equipmentID = equipment.equipmentID " + "         LEFT JOIN reserveCart" + "                   ON equipment.equipmentID = reserveCart.equipmentID " + "WHERE (equipment.status = 'Available' OR equipment.status = 'CheckedOut') ";
...
//                sql = "SELECT * " + "FROM wishlist" + " RIGHT JOIN" + "  equipment" + "     ON" + " wishlist.equipmentID = equipment.equipmentID " + "WHERE (status = 'Available' OR status = 'CheckedOut')" + "  AND equipmentType = ?" + "  AND (userID = ? OR userID IS NULL) and private = 0 and campusID=?";
                sql = "SELECT * " + "FROM wishlist\n" + "         RIGHT JOIN equipment" + "                    ON wishlist.equipmentID = equipment.equipmentID " + "         LEFT JOIN reserveCart" + "                   ON equipment.equipmentID = reserveCart.equipmentID " + "WHERE (equipment.status = 'Available' OR equipment.status = 'CheckedOut') " + "  AND equipmentType = ?" + " AND isPrivate = 0 ";
//                        "  AND equipment.campusID = ?";
            } else {
//                sql = "SELECT * " + "FROM wishlist" + " RIGHT JOIN" + "  equipment" + "     ON" + " wishlist.equipmentID = equipment.equipmentID " + "WHERE (status = 'Available' OR status = 'CheckedOut')" + "  AND equipmentType = ?" + "  AND (userID = ? OR userID IS NULL) and campusID=?";
                sql = "SELECT * " + "FROM wishlist" + "         RIGHT JOIN equipment" + "                    ON wishlist.equipmentID = equipment.equipmentID " + "         LEFT JOIN reserveCart" + "                   ON equipment.equipmentID = reserveCart.equipmentID " + "WHERE (equipment.status = 'Available' OR equipment.status = 'CheckedOut') " + "  AND equipmentType = ?";
//                        "  AND (wishlist.userID = ? OR wishlist.userID IS NULL)" +
//                        "  AND equipment.campusID = ?";
...
//            ps.setString(2, campus);
    ...
    // Technician  equipment action
    public boolean updateEquipment(int equipmentID, String name, String type, String status, String campus, String pDate, String wPeriod, int isPrivate) {
        boolean result = false;
        PreparedStatement pStmt = null;
        Connection conn = null;
        try {
            conn = getConnection();
            String sql = "UPDATE equipment SET equipmentName = ?, equipmentType = ?, status = ?, campusID = ?, purchaseDate = ?, warrantyPeriod = ?,isPrivate = ? WHERE equipmentID = ?";
            pStmt = conn.prepareStatement(sql);
            ...
            pStmt.setString(6, wPeriod);
            pStmt.setInt(7, isPrivate);
            pStmt.setInt(8, equipmentID);
